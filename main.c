...
{
int i;
int p_gross=400,h_gross; BSTree T=NULL; BSTree H=NULL;
srand((unsigned) time(NULL)); system("mode con: cols=150 lines=160"); do{
printf("	Please input your option:"); menu();
...
switch(i){
	...
	case 4:{//set remove
		int key;
		printf("Please input id:"); scanf("%d",&key);getchar(); printf("\n");
	...
	case 5:{//set intersection
		char filename[20]; BSTree T1=NULL; BSTree T0=NULL;
		printf("Please input filename:"); scanf("%s",filename);getchar(); printf("\n"); 
		if(load_data(&T1,filename)){
			printf("Successed to load!\n"); set_intersection(T,T1,&T0); set_init(&T);
			T=T0;
			height(T,0);
	...
	case 6:{//set union
		char filename[20]; BSTree T1=NULL;
		printf("Please input filename:"); scanf("%s",filename);getchar(); printf("\n"); 
		if(load_data(&T1,filename)){
			printf("Successed to load!\n"); 
			set_union(&T,T1); 
	...
	case 7:{//set difference
		...
		printf("\n"); 
		if(load_data(&T1,filename)){
			printf("Successed to load!\n"); set_difference(&T,T1); height(T,0);
			printf("Successed to union!\n");
		}
		else
			printf("Failed to load!\n");
	...
	case 15:{//create random need 3 files given last hobby
		BSTree T0;
...
void set_destory(BSTree *T)
{
    ...
    if((*T)!=NULL){
        set_destory(&((*T)->lchild)); set_destory(&((*T)->rchild)); free(*T);
    }
}
    ...
    if(!(*T)){
        ...
        *taller=true;
        ...
        if(e.id<(*T)->data.id){
            ...
            if(!set_insert(&((*T)->rchild),e,taller))return false; if(*taller)
                switch((*T)->bf){ 
				case LH:
                        (*T)->bf=EH;
						*taller=false;
...
bool set_remove(BSTree *T,int key,bool *shorter)//delete
{
...
void set_intersection(BSTree T,BSTree T1,BSTree *T0)//if the node is both in T1 and T, add to T0
{
...
void set_union(BSTree *T,BSTree T1)//simply adding T1 to T, don't considerate if there are some same nodes
{
    if(T1==NULL)
		return; 
...
void set_difference(BSTree *T,BSTree T1)//remove every node that's in T1 and T, finally T consists of every node that T1 doesn't have
{
    if(*T==NULL||T1==NULL)
		return; 
...
bool set_is_member(BSTree T,int key,BSTree *Ts)//find a node with key in T, if found, Ts will point at it, or, Ts will be NULL
{
    if(T==NULL)
		return false; 
	if(T->data.id==key){
...
bool set_is_subset(BSTree T,BSTree T1)//find out if T1 is a subset of T
{
...
bool set_is_equal(BSTree T,BSTree T1)//find out if T1 equals to T
{
    if(!set_is_subset(T,T1))
		return false; 
...
bool load_data(BSTree *T,char *filename)//read the data in file one by one
{
    BSTree p;
    FILE *fp;
...
bool save_data(BSTree T,FILE *fp)//save the data in file one by one from top to down
{
    if(T){
...
void height(BSTree T,int i)//save its relative height in the Troot node
{
    if(T)
		T->h=i+1;
    else 
...
void graph(BSTree T,int x,int y,visit fp)//pre-order-traverse
{

    if(T){
        gotoxy(x+x0,y+y0); fp(T);
    }
    else return;
    if(T->lchild||T->rchild){ gotoxy(x+x0,y+y0+1); printf("©¦");
        if(T->lchild){
...
void operate_id(BSTree T)//show T's id
{
    printf("%d",T->data.id);
}

void operate_relation(BSTree T1,BSTree T)//if T1root node id is in T, show its id last name given name
{
    if(T1==NULL)
		return;
...
void operate_hobby(BSTree T1,BSTree H)//if T1root node id is in H, show its id given name
{
    if(T1==NULL)
		return;
...
bool person_input(BSTree *T,int p_gross)//need lastname.txt givenname.txt
{
...
bool hobby_set_input(BSTree *H,int *h_gross)//need hobby.txt
{

    FILE *fp;
    Info *q;
    int number=0; if((fp=fopen("hobby.txt","r"))==NULL){
...
void indirect_friends_traverse(BSTree T,BSTree Tf,BSTree *T0)
{
    if(Tf==NULL)
		return;
	set_is_member(T,Tf->data.id,&Ts);//find Tf's id in T, if so, Ts points at that node in T
...
void complete_traverse(BSTree T,BSTree T0)//complete itself
{
    if(T0==NULL)
		return;
...
void complete_friends(BSTree T,BSTree T1,BSTree Tf)//complete the friend relationship
{
    if(Tf==NULL)
		return;
    BSTree Ts0;
